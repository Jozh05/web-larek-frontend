...
IProductItem – Описывает товар с полями ID, названием, описанием, изображением, категорией и ценой.

IActions – Определяет обработчик клика по элементу.

IOrderForm – Интерфейс формы заказа, содержащий платежные данные, адрес, телефон, email и итоговую сумму.

IOrder – Расширяет IOrderForm, добавляя массив ID товаров.

IOrderLot – Полноценный заказ с обязательными полями (платёж, email, телефон, адрес, сумма и товары).

IOrderResult – Результат оформления заказа с ID и итоговой суммой.

FormErrors – Тип, описывающий возможные ошибки в полях формы заказа.
